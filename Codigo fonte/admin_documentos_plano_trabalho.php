...
if(isset($_POST['editar'])){
	...
	if(($_POST['id'])and(isset($_FILES["documento"]))){
		...
		if($nome_temporario != ''){
			...
			$documento = $DocumentosPlanoTrabalho->find($id);
			...
			if($documento->dpt_caminho != ''){
				
				unlink($documento->dpt_caminho);    
			
			}
			
			    //realiza a busca das informações do curso
			$curso = $Cursos->find($documento->dpt_cursos);
			...
			$caminho = str_replace($comAcentos, $semAcentos, "Documentacao/Coordenacao/".$resultado->doc_ano ."/Planos_de_Trabalho/".$curso->c_titulo."_".$documento->dpt_ciclo.".".$pathinfo['extension']);
			copy($nome_temporario, $caminho);
			
			if(!($DocumentosPlanoTrabalho->setCaminho($caminho))){
						
				die('Falha ao tentar realizar o cadastro, limite de caracteres ultrapassado.');
					
			}
		   
			if($DocumentosPlanoTrabalho->update($id)){
...
//desativa e ativa dados de documentos do plano de trabalho
if(isset($_POST['cursos'])){
	
		//Deleta todas as ligacoes deste vestibulinho
	if(!$DocumentosPlanoTrabalho->deleteAll($resultado->doc_id)){
   	...
   	if(isset($_POST['curso_plano_trabalho'])){  
   			
			//Cadastra as novas ligacoes
		foreach($_POST['curso_plano_trabalho'] as $key => $curso):
			
			$documentos = $DocumentosPlanoTrabalho->findAll($resultado->doc_id, $curso, 'N');
			...
			else
			{
				...
				if(!($DocumentosPlanoTrabalho->delete($resultado->doc_id, $curso, 'S'))){
...
function cadastrar($id, $periodo, $ano){
	...
	for($i = 0; $i < $cadastrar; $i++){
		...
		$DocumentosPlanoTrabalho->setCurso($id);
		$DocumentosPlanoTrabalho->setAno($ano);
...
<div class="etec-admin-pad-barlateral">
   ...
   <div class="etec-admin-pad-bar-info"><i>Informativo:</i>  Nesta página contém informações referente ao plano de trabalho.</div>
    <a href="admin_coordenacao.php" ><div class="etec-admin-pad-bar-iten etec-admin-pad-bar-iten-btn " >
   	Voltar
   	</div></a>
    <a href="admin_documentos_plano_trabalho_cadastro.php" ><div class="etec-admin-pad-bar-iten etec-admin-pad-bar-iten-btn " >
   	Cursos
   	</div></a>
...
?>
<a href="admin_documentos_plano_trabalho_editar.php?A44dsddERTjsJfdJLsSrEUd566457IfdKddItfAfnNB57hyDVSa56JgSaKPd3U4SFfa4434fdsdsttv5fd3DdfDdDS45S45SDF3=<?php echo $value_documento->dpt_id; ?>" >       
 <div class="etec-admin-pad-conteudo-unit admin-pad-unit-ativar-color">
   ...
   <div class="etec-admin-pad-cont-unit-info">
     <?php  echo $value_documento->dpt_ciclo; ?>
   </div>
   ...
   <div class="etec-admin-pad-cont-unit-info-reduces etec-admin-pad-cont-unit-info " <?php if($value_documento->dpt_caminho==""){echo 'align="center"';}?>>
      <?php 
	  
	  	if($value_documento->dpt_caminho==""){
			...
			echo $value_documento->dpt_caminho;
